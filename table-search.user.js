...
(function() {
    ...
    function showUpdateNotification(latestVersion) {
        ...
        modal.innerHTML = `
            ...
            ">
                <h2 style="margin: 0 0 20px 0; color: ${colors.primary}; font-size: 24px;">
                    🎉 Update Available!
                </h2>
                <p style="margin: 0 0 20px 0; color: ${colors.text}; font-size: 16px;">
                    A new version of Smartsheet Full Table Search is available.
                </p>
                <div style="background: #f5f5f5; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                    ...
                    <div>
                        <strong>Latest Version:</strong> <span style="color: ${colors.accent};">${latestVersion}</span>
                    </div>
                </div>
                <div style="display: flex; gap: 10px;">
                    ...
                    <button id="remind-later-btn" style="
                        ...
                        cursor: pointer;
                    ">Remind Later</button>
                    <button id="skip-version-btn" style="
                        flex: 1;
                        padding: 12px;
                        background: #f0f0f0;
                        color: ${colors.text};
            ...
            GM_xmlhttpRequest({
                ...
                onload: function(response) {
                    try {
                        ...
                        if (isNewerVersion(latestVersion, CURRENT_VERSION) && latestVersion !== skippedVersion) {
    ...
    async function fetchAllRowsFromAPI() {
        ...
        try {
            ...
            if (data.rows && data.rows.length > 0) {
                ...
                console.log('✅ Successfully loaded ALL rows from API:', allTableData.length);
                apiDataLoaded = true;
                return true;
            }
    ...
    function createSearchUI() {
        ...
        container.innerHTML = `
            <style>
                ...
                #wf-check-update-icon:hover {
                    background-color: rgba(255, 255, 255, 0.2);
                }

                #wf-check-update-icon:disabled {
                    opacity: 0.5;
                    cursor: not-allowed;
                }
            ...
            <div id="wf-search-header">
                <div id="wf-search-title">
                    <div id="wf-search-logo">🔍</div>
                    <span>Full Table Search</span>
                </div>
                <div style="display: flex; gap: 8px; align-items: center;">
                    <button id="wf-check-update-icon" title="Check for updates">🔄</button>
                    <button id="wf-search-toggle">−</button>
                </div>
            </div>
            ...
            return `
                <div class="wf-result-item" data-row-index="${result.rowIndex}" data-search-value="${query}" style="cursor: pointer;">
                    <div style="font-weight: 600; color: ${colors.primary}; margin-bottom: 8px;">
                        📋 Row ${result.rowIndex + 1} <span style="font-size: 11px; color: #666;">- Click to jump</span>
                    </div>
                    ${allCellsHtml}
                </div>
            `;
    ...
    function init() {
        ...
        const checkUpdateIcon = document.getElementById('wf-check-update-icon');
        ...
        apiLoadBtn.addEventListener('click', async () => {
            ...
            apiLoadBtn.textContent = '⏳ Loading...';
            
            const success = await fetchAllRowsFromAPI();
            
            if (success) {
                updateStats();
                apiLoadBtn.textContent = '✅ Data Loaded!';
                apiLoadBtn.style.background = 'linear-gradient(135deg, #40B14B 0%, #2d8a37 100%)';
                setTimeout(() => {
                    apiLoadBtn.textContent = '🔄 Reload All Data';
                    apiLoadBtn.disabled = false;
                }, 2000);
            } else {
                apiLoadBtn.textContent = '❌ Load Failed';
                apiLoadBtn.disabled = false;
        ...
        checkUpdateIcon.addEventListener('click', () => {
            checkUpdateIcon.disabled = true;
            checkUpdateIcon.textContent = '⏳';
            checkForUpdates(true);
            setTimeout(() => {
                checkUpdateIcon.textContent = '🔄';
                checkUpdateIcon.disabled = false;
            }, 2000);
        });
